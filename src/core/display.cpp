...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    drawStatusBar();
    int menuSize = options.size();
    bool compactScreen = tftHeight <= 80;
    tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);
    tft.setTextSize(FP);
    tft.drawPixel(0, 0, 0);
    int titleY = compactScreen ? 27 : 30;
    tft.fillRect(6, titleY, tftWidth - 12, 8 * FP, bruceConfig.bgColor);
    tft.drawString(title, 12, titleY);

    // middle of the drawing area
    int middle = 25 /*status*/ + (tftHeight - 30 /*status + bottom margin*/) / 2;
    // Very short displays like 160x80 need tighter but explicit spacing, or the
    // title/previous/current/next lines end up drawing into each other.
    int middle_up = compactScreen ? (titleY + 11) : middle - (tftHeight - 42) / 3 - FM * LH / 2 + 4;
    int middle_down = compactScreen ? (middle + 13) : middle + (tftHeight - 42) / 3 - FM * LH / 2;
    ...
    // Selected item
    int selectedTextSize = compactScreen ? FM : (options[index].label.length() <= tftWidth / (LW * FG) - 1 ? FG : FM);
    tft.setTextSize(selectedTextSize);
    tft.setTextColor(bruceConfig.priColor);
    int selectedBoxHeight = compactScreen ? (FM * LH + 4) : (FG * LH + 5);
    int selectedBoxY = compactScreen ? (middle - selectedTextSize * LH / 2 - 2) : (middle - FG * LH / 2 - 1);
    tft.fillRect(6, selectedBoxY, tftWidth - 12, selectedBoxHeight, bruceConfig.bgColor);
