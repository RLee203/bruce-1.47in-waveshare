...
backup_file = "%s.old" % (original_file)
...
def _find_package_dir(*package_names):
    for package_name in package_names:
        try:
            package_dir = env.PioPlatform().get_package_dir(package_name)
        except KeyError:
            package_dir = None
        if package_dir and exists(package_dir):
            return package_dir
    return None
...
def _patch_net80211():
    ...
    if mcu == "esp32c5" or mcu == "esp32c6":
        toolchain_dir = _find_package_dir("toolchain-riscv32-esp")
        objcopy = join(toolchain_dir, "bin", "riscv32-esp-elf-objcopy.exe")
    else:
        xtensa_package = "toolchain-xtensa-%s" % mcu
        toolchain_dir = _find_package_dir(xtensa_package, "toolchain-xtensa-esp-elf")
        if toolchain_dir and exists(join(toolchain_dir, "bin", "xtensa-%s-elf-objcopy.exe" % mcu)):
            objcopy = join(toolchain_dir, "bin", "xtensa-%s-elf-objcopy.exe" % mcu)
        else:
            objcopy = join(toolchain_dir, "bin", "xtensa-esp-elf-objcopy.exe")

    if not toolchain_dir or not exists(objcopy):
        print("Patch: objcopy tool not found")
        return
    ...
    if isfile(patched_file):
        if isfile(original_file):
            if isfile(backup_file):
                remove(backup_file)
            rename(original_file, backup_file)
        elif not isfile(backup_file):
        ...
        if not isfile(original_file) and isfile(source_file):
            shutil.copy2(source_file, original_file)

needs_patch = not isfile(original_file) or not isfile(patchflag_path)
if needs_patch and mcu != "":
    _patch_net80211()
